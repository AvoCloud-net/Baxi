...
def minigame_commands(bot: commands.AutoShardedBot):
    import asyncio
    import assets.games.counting as counting_game
    import assets.games.quiz as quiz_game

    logger.debug.info("Minigame commands loaded.")
    ...
    @counting_group.command(name="score", description="Show current count and high score")
    async def counting_score(interaction: discord.Interaction):
        await interaction.response.defer()
        if interaction.guild is None:
            return
        lang = datasys.load_lang_file(interaction.guild.id)
        t: dict = lang["games"]["counting"]
        data: dict = dict(datasys.load_data(interaction.guild.id, "counting"))
        if not data.get("enabled", False):
            await interaction.edit_original_response(content=t["score_not_enabled"])
            return
        current = int(data.get("current_count", 0))
        high = int(data.get("high_score", 0))
        ch = interaction.guild.get_channel(int(data.get("channel", 0)))
        embed = discord.Embed(
            title=t["score_title"],
            description=str(t["score_description"]).format(
                current=current,
                high=high,
                channel=ch.mention if ch else t["score_no_channel"],
            ),
    ...
    @counting_group.command(name="reset", description="Reset the count to 0 (Admin)")
    ...
    async def counting_reset(interaction: discord.Interaction):
        ...
        t: dict = lang["games"]["counting"]
        data: dict = dict(datasys.load_data(interaction.guild.id, "counting"))
        old = int(data.get("current_count", 0))
        data["current_count"] = 0
        data["last_user_id"] = 0
        datasys.save_data(interaction.guild.id, "counting", data)
        embed = discord.Embed(
            description=str(t["reset_done"]).format(old=old),
            color=config.Discord.warn_color,
        )
        embed.set_footer(text=t["footer"])
        await interaction.edit_original_response(embed=embed)
    ...
    flagquiz_group = app_commands.Group(
        name="flagquiz",
        description="Flag Quiz",
    )

    @flagquiz_group.command(name="leaderboard", description="Show the Flag Quiz leaderboard")
    async def flagquiz_leaderboard(interaction: discord.Interaction):
        await interaction.response.defer()
        if interaction.guild is None:
            return
        lang = datasys.load_lang_file(interaction.guild.id)
        t: dict = lang["games"]["flag_quiz"]
        lb = quiz_game.get_leaderboard(interaction.guild.id, bot)
        if not lb:
            embed = discord.Embed(
        ...
        medals = ["🥇", "🥈", "🥉"]
        lines = []
        for i, (name, pts) in enumerate(lb[:10]):
            prefix = medals[i] if i < 3 else f"**{i + 1}.**"
            pts_label = t["leaderboard_pts_single"] if pts == 1 else t["leaderboard_pts_plural"]
    ...
    bot.tree.add_command(flagquiz_group)
