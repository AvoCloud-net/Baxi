...
async def on_guild_join_event(guild: discord.Guild, bot):
    ...
    if str(guild.id) in banned_server:
        ...
        await channel.send(
            ...
            .set_thumbnail(url=icons_url + "hi.png")
            .set_footer(text="PyroPixle"),
            view=InviteUndWebUndDiscordundDocsButton(),
        )

        await bot.get_channel(1175821691860033536).send(
            embed=discord.Embed(
                title="Ein neuer Server!",
                description=f"Der Server `{guild.name}` nutzt nun Baxi!",
                color=embedColor,
...
async def on_member_join_event(member: discord.Member, bot):
    ...
    if get_user.flagged:
        ...
        try:
            await member.guild.get_channel(
                log_channels[str(member.guild.id)]["channel_id"]
            ).send(embed=embedwarn)
    ...
    if str(member.guild.id) in welcomelist:
        ...
        embed = discord.Embed(
            ...
            color=color_mapping.get(guild_color),
        ...
        await bot.get_channel(
            int(welcomelist[str(member.guild.id)]["channel_id"])
        ).send(embed=embed)
    ...
    try:
        ...
        try:
            if str(channel.guild.id) in log_channels:
                ...
                embed = discord.Embed(
                    title=language["log_channel_title"],
                    description=language["log_channel_msg_edit"].format(
                ...
                await guild.get_channel(
                    log_channels[str(channel.guild.id)]["channel_id"]
                ).send(embed=embed)
...
async def audit_log_entry(entry, action_counts):
    try:
        ...
        if action == discord.AuditLogAction.kick:
            await log_action(
                entry.guild.id, executor.id, "member_kick", action_counts=action_counts
            )
            logger.info("Member KICK")
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_member-kick"].format(
                    user=target.name, reason=entry.reason, executer=executor.mention
                ),
                color=embedColor,
            ...
            await log_action(
                entry.guild.id, executor.id, "member_ban", action_counts=action_counts
            )
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_member-ban"].format(
                    user=target.name, reason=entry.reason, executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_member-ban_remove"].format(
                    user=target.name, executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_member-update_perms"].format(
                    user=target.mention, executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_member-moved_vc"].format(
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_bot-add"].format(
                    executer=executor.mention, bot=target.user.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_channel_create"].format(
                    channel=target.mention, executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_channel-update"].format(
                    channel=target.mention, executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_channel-delete"].format(
                    channel=entry.changes.before.name, executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_msg-delete"].format(
                    user=target.name,
                    executer=executor.mention,
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_msg-delete_bulk"].format(
                    user=target.name,
                    executer=executor.mention,
                    channel=entry.target.channel.mention,
                    count=str(entry.extra.count),
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_msg-pin"].format(
                    user=target.mention,
                    executer=executor.mention,
                    channel=entry.target.channel.mention,
                ),
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_msg-unpin"].format(
                    user=target.mention,
                    executer=executor.mention,
                    channel=entry.target.channel.mention,
                ),
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_invite-create"].format(
                    executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_invite-delete"].format(
                    executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_emoji-create"].format(
                    executer=executor.mention, emoji=target.name
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_emoji-delete"].format(
                    executer=executor.mention, emoji=entry.changes.before.name
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_sticker-create"].format(
                    executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_sticker-delete"].format(
                    executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_guild-update"].format(
                    executer=executor.mention
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_webhook-create"].format(
                    executer=executor.mention, integration=target.name
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
            ...
            embed = discord.Embed(
                title=language["log_channel_title"],
                description=language["log_webhook-delete"].format(
                    executer=executor.mention, integration=entry.changes.before.name
                ),
                color=embedColor,
            ).set_thumbnail(url=executor.avatar.url)
