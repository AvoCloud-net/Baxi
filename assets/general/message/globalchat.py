...
async def get_user_badges(user_id, bot):  # noqa
    ...
    staff_badge = (
        "<:mod:1244728840803192933>"
        if user_id in botadmin or user_id in staff_user
        else ""
    )
    early_support_badge = (
        "<:boost:1244730261997097032>"
        if (user_id in early_support_user or user_id in projectsupporter_user)
        else ""
    )
    dev_badge = "<:code:1244728988186710046>" if user_id in dev_user else ""
    active_badge = (
        "<:rocketlunch:1244729975899422783>" if user_id in active_user else ""
    )
    verified_badge = "<:verify:1244723476825243718>" if user_id in verified else ""
    owner_roles_badge = (
        "<:owner:1244730117629149355>" if user_id == 686634546577670400 else ""
    )  # RED_WOLF USER
...
async def handel_gc(
    booted: bool, emergency_mode: bool, message: discord.Message, user_api, bot
):
    ...
    discord_img_pattern = (
        r"https?://(?:images-ext-\d\.discordapp\.net/external|cdn\.discordapp\.com)/.*"
    )
    ...
    try:
        ...
        (
            ...
            on_pyropixle_badge,
            verified_badge,
        ) = await get_user_badges(message.author.id, bot)
        grole = get_user_role(message.author.id)

        if (
            owner_roles_badge == ""
            and staff_badge == ""
            and early_support_badge == ""
            and active_badge == ""
            and on_pyropixle_badge == ""
            and dev_badge == ""
            and verified_badge == ""
        ):
            badge_abteil = ""
        ...
        embed = (
            discord.Embed(
                title=f"<:user:1244726847401492590> 〢 {message.author.name}",
                description=(
            ...
            .set_author(name=f"{grole}", icon_url=get_user_role_icon(message.author.id))
            .set_thumbnail(url=user_avatar)
        )
        ...
        if message.reference is not None:
            try:
                ...
                if replied_message and replied_message.embeds:
                    ...
                    original_msg_id = referenced_embed.footer.text.split("|")[
                        2
                    ].strip()[5:]
                    embed.set_footer(
                    ...
                    embed.add_field(
                        ...
                        value=new_content_answer_message,
                        inline=False,
                    )
                    ...
                    embed.set_footer(
                        text=f"Sent from planet {message.guild.name} | UID: {message.author.id} - SID: "
                        f"{message.guild.id} | MID: {generated_msg_id}",
                        icon_url=server_icon,
                    )
        ...
        elif discord_image_link is not None:
                        ...
                        if (
                            ...
                            if message.content.startswith("admin.botg"):
                                if message.author.id in botadmin:
                                    await channel.send(
                                        embed=discord.Embed(
                                            title="<:user:1244726847401492590> 〢 BAXI",
...
async def delete_gc_messages_cmd(interaction: discord.Interaction, msg_id: str, bot):
    try:
        ...
        if msg_id in gc_messages:
            if (
                interaction.user.id == gc_messages[msg_id]["user_id"]
                or interaction.user.id in botadmin
            ):
                for reply_info in gc_messages[msg_id]["replies"]:
                    try:
                            ...
                            if channel:
                                try:
                                    try:
                                        ...
                                        logger.warn(
                                            f"Error fetching reply {reply_info['message_id']}: {e}"
                                        )
                                        continue
                        ...
                        await interaction.edit_original_response(
                            content=language["unknown_error"] + str(e)
                        )
                        return
                ...
                save_data("json/gc_messages.json", gc_messages)
                await interaction.edit_original_response(
                    content=f"{language['delete_success']}: {deleted_count}"
                )
            else:
                await interaction.edit_original_response(
